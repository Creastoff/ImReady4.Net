%%%%%%%
% VERSION 4.43 (10/07/2023)
%%%%%%%

function ImReady4()
    %Delete already existing figures
    currentVersion = "4.43";
    availableUpdateExists = searchForAppUpdates(currentVersion);
    ...
    disp(['Training Advice created on: ']);
    disp(" ");
    ...
    for j = firstAthlete:lastAthlete
        ...
        initDate = datestr(addtodate(now(), - 89, 'days'), formatOut);
        url = strcat('https://intervals.icu/api/v1/athlete/', ID,
        ...
        fullFilePath = 'C:\Users\Kristof\source\repos\ImReady4\ImReady4Tests\SuccessfulTestData.csv';
        [A, B, C, D] = textread(fullFilePath,'%s,%d,%f,%f','delimiter', ',', 'headerlines', 1);
        resp = struct("date", A,
                      settings.FIELD_RHR, B,
                      settings.FIELD_rMSSD, C,
                      settings.FIELD_SDNN, D);

        [numDays, ~] = size(resp);

        RHR = resp.(settings.FIELD_RHR);

        rMSSD = resp.(settings.FIELD_rMSSD);
        HRV_rMSSD = 20*log(rMSSD);

        SDNN = resp.(settings.FIELD_SDNN);
       ...
       % view(0, 60)
        ...
        if not(isnan(HRV_rMSSD(1))) && c1 == 7
            d1{1} = 'No variability in last 30 days';
            d1{2} = 'Take more measurements';
        end
...
function settings = readSettings()
    data = fullfile('config_athletes.m' );
